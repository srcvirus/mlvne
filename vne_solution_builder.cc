...
void VNESolutionBuilder::PrintMapping(const OverlayMapping *vne,
                                      const char *nmap_filename,
                                      const char *emap_filename) {
  ...
  for (vne_it = vne->edge_map.begin(); vne_it != vne->edge_map.end();
    ...
    for (int i = 0; i < mapped_path.size(); ++i) {
      printf("Virtual link (%d, %d) --> IP link (%d, %d, %d)\n", vlink.first,
             vlink.second, mapped_path[i].first, mapped_path[i].second,
             mapped_path[i].order);
      if (emap_outfile) {
        fprintf(emap_outfile,
...
unique_ptr<OverlayMapping> VNESolutionBuilder::TranslateEmbeddingToIP(
  ...
  for (vne_it = vne->edge_map.begin(); vne_it != vne->edge_map.end();
    ...
    for (int i = 0; i < paths.size(); ++i) {
      ...
      ip_topology->AddEdge(ip_link.first, ip_link.second, bandwidth, 1,
...
void VNESolutionBuilder::PrintNewIPLinks(const OverlayMapping *vne,
                                         const char *filename) {
  FILE *outfile;
  if (filename) outfile = fopen(filename, "w");
  if (vne == NULL) return;
