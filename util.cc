...
//     const Graph* phys_topology, const ptr_vector<OverlayMapping>& embeddings,
//     int num_vns) {
...
unsigned long GetBandwidthCost(
  ...
  for (int i = 0; i < virt_topologies.size(); ++i) {
    const OverlayMapping& embedding = vn_embeddings[i];
...
//         long b_mn = virt_topology.GetEdgeBandwidth(vlink.first,
//         vlink.second);
...
//     for (end_point_it = u_neighbors.begin(); end_point_it !=
//     u_neighbors.end();
//          ++end_point_it) {
...
void ComputePhysicalNetworkCapacity(
  ...
  for (int i = 0; i < virt_topologies.size(); ++i) {
    ...
    const OverlayMapping& vne = vn_embeddings[i];
    edge_map_t::const_iterator emap_it;
...
void WriteSolutionToFile(const boost::ptr_vector<OverlayMapping>& vn_embeddings,
  ...
  for (int a = 0; a < vn_embeddings.size(); ++a) {
    ...
    const OverlayMapping& vne = vn_embeddings[a];
    ...
    for (emap_it = vne.edge_map.begin(); emap_it != vne.edge_map.end();
      ...
      for (pit = plinks.begin(); pit != plinks.end(); ++pit) {
        fprintf(emap_f, "Virtual edge (%d, %d) --> physical edge (%d, %d)\n",
